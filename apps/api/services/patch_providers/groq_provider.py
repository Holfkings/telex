...
Default model: llama-3.3-70b-versatile (Groq's fastest large model as of 2025;
change _DEFAULT_MODEL to swap). Groq uses an OpenAI-compatible API.
Requires: groq>=0.9.0 package.
"""
import logging
...
# Update this constant to change the default Groq model used for patch generation.
_DEFAULT_MODEL = "llama-3.3-70b-versatile"


class GroqProvider(PatchProvider):
    ...
    async def generate_patch(
        ...
        try:
            ...
            logger.error("GroqProvider.generate_patch failed: %s", exc)
            raise
    ...
    async def classify_failure(
        ...
        try:
            ...
            logger.error("GroqProvider.classify_failure failed: %s", exc)
            raise
